...
# ---------------------------------------------------------------------------
# ATS Boost Prompt — post-tailoring optimization pass
# ---------------------------------------------------------------------------

ATS_BOOST_PROMPT = """You are an expert ATS resume optimizer. Given this resume and its current ATS analysis, output ONLY the targeted changes needed to push the score above 90.

CURRENT ATS SCORE: {current_score}/100
MISSING JD KEYWORDS (not yet in resume): {missing_keywords}
...
Do NOT inject highly domain-specific keywords (industry names, regulations like HIPAA, product names) into bullets unless the candidate's experience clearly supports it — instead, add those only to technicalSkills if appropriate.

RULES:
- Each change MUST include "original" with the EXACT verbatim text at that path (for verification)
- Do NOT change company names, institution names, job titles, dates, or degree names
- Do NOT use em dash characters
- Prefer targeted single-bullet changes over full-section rewrites
- If a bullet is already good, skip it
...
{{
  "changes": [
    {{
      ...
      "reason": "added action verb / added quantification / injected keyword X"
    }},
    {{
      "path": "additional.technicalSkills",
      "action": "reorder",
      "original": null,
      "value": ["all existing skills...", "plus any new keyword"],
      "reason": "added missing keyword: voice AI systems"
    }}
  ]
}}"""
